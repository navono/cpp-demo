...
class XLogger {
 ...
 private:
  ...
  XLogger() {
    ...
    const std::string logger_name_prefix = "cppTemplate_";
    ...
    try {
      ...
      if (console) {
        ...
        auto file_sink = std::make_shared<spdlog::sinks::rotating_file_sink_mt>(log_dir + "/" + logger_name + ".log", 5 * 1024 * 1024, 1000);// multi part log files, with every part 500M, max 1000 files
        ...
        m_logger = spdlog::create_async<spdlog::sinks::rotating_file_sink_mt>(logger_name, log_dir + "/" + logger_name + ".log", 5 * 1024 * 1024, 1000);// multi part log files, with every part 500M, max 1000 files
      }
